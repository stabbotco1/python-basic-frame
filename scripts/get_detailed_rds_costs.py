...
def get_rds_cost_breakdown(start_date, end_date, by_day=False, by_region=False):
    """
    Get the cost breakdown for RDS service by resource.
    ...
    try:
        ...
        # Explicitly set the granularity to DAILY if --by-day is used
        ...
        df = df.sort_values(by=['Region', 'Service', 'ResourceID', 'Date'], ascending=[True, True, True, True])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='AWS RDS Cost Breakdown Script')
    ...
    print(f"Fetching AWS RDS cost data from {start_date} to {end_date}...")
    if args.by_day:
        print("Including daily breakdown.")
    if args.by_region:
        print("Including regional breakdown.")
    
    # Get the RDS cost breakdown
    cost_df = get_rds_cost_breakdown(start_date, end_date, by_day=args.by_day, by_region=args.by_region)
    ...
    if cost_df is not None:
        ...
        csv_filename = 'aws_rds_cost_breakdown.csv'
